...
# LL1 <- function(par, xbar, Sx, r01, p, delta = 0) # { mu = par[1] sigma = par[2] phi = pnorm( (delta-mu)/sigma) log(sigma) + (Sx + (xbar -
...
ztruncnorm <- function(dat, cutoff = 0.1, iter = 200, s_upper = 4, s_lower = 0.1, p_min = 0.6, p_max = 1) {
    ...
    # first fit truncated normal distribution using only positive entries write the log-likelihood in terms of only std and optimize by Brent
    ...
    param1 <- foreach(g = c(ind, ind2), .combine = rbind) %dopar% {
        ...
        tryCatch({
            res <- optim(par = sqrt(var[g]), fn = LLwZ, xbar = mu[g], Sx = var[g], r01 = r01[g], p = pg[g], delta = cutoff, method = "Brent", upper = s_upper,
