...
SIMPLE <- function(dat, K0, M0 = 1, iter = 10, est_lam = 1, impt_it = 5, penl = 1, sigma0 = 100, pi_alpha = 1, beta = NULL, verbose = F, max_lambda = F, lambda = NULL,
    sigma = NULL, mu = NULL, est_z = 1, clus = NULL, p_min = 0.8, cutoff = 0.1, K = 10, min_gene = 300, num_mc = 3, fix_num = F, mcmc = 50, burnin = 2) {
    ...
    if (fix_num) {
        ...
        if (length(hq_ind) < min_gene)
            hq_ind <- order(n1, decreasing = T)[1:min_gene]  # fix number of hq genes for simulation, need to change back
    }
