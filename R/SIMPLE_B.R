# Factor mixture model, laplace prior for B integrate bulk
...
#'   \eqn{\pi~Dir(\alpha)}. The algorithm first runs Monte Carlo EM using only
...
#'   Carlo EM using all the genes. In the initial phase, we used the genes with
#'   dropout rate less than \emph{1 - p_min}; if the number of genes is less
#'   than \emph{min_gene}, we ranked the genes by the number cells with nonzero
#'   expression and kept the top \emph{min_gene} genes.
...
#' result <- scimpclu_bulk(Y2, K0, data.frame(simu_data$bulk), M0, celltype=rep(1, n), clus = NULL, K = 20, p_min = 0.5, min_gene = 200,cutoff=0.01, max_lambda=T) \cr
#' \cr
...
SIMPLE_B <- function(dat, K0, bulk, M0 = 1, celltype = NULL, clus = NULL, K = 20, iter = 10, est_z = 1, impt_it = 5, max_lambda = F, est_lam = 1, penl = 1,
    ...
    # random start
    if (is.null(lambda))
        lambda <- matrix(1/M0, M0, K0)  # sum M0
    ...
    # init clustering
    if (is.null(clus)) {
        # s = svd(t(scale(t(res)))) km0 <- kmeans(s$v[,1:K], M0, iter.max = 80, nstart = 300)
    ...
    # V = impute_hq$Varf[[1]] ML <- chol(V) lq_ind = which(n1 < p_min )
    ...
    res <- foreach(g = lq_ind, .combine = rbind) %dopar% {
        ...
        # #s_ind = which(dat[g, ] > cutoff) W_temp = impute_hq$Ef[[1]][, ,drop=F] #s_ind Y_temp = dat[g, ] #res[[1]][g, ], s_ind

        # ns = n #length(s_ind) W_aug <- rbind(W_temp, ML*sqrt(ns)) #(n+K) * K Y_aug <- c(Y_temp, rep(0,K0)) #G*(n+K)

        penalty <- penl/(M0 * n + K0)/var(Y_aug)  # sigma^2
        fit1m <- glmnet(W_aug, Y_aug, family = "gaussian", alpha = 1, intercept = F, standardize = F, nlambda = 1, lambda = penalty * K0/(M0 + K0), penalty.factor = c(rep(0,
    ...
    impute <- matrix(0, n, G)
    if (iter > 0) {
        # EM for all genes Y[hq_ind, ] = Y[hq_ind, ] + impute_hq$geneM
        ...
        return(list(loglik = impute_result$loglik, pi = impute_result$pi, mu = impute_result$mu, sigma = impute_result$sigma, beta = impute_result$beta, lambda = impute_result$lambda,
            z = impute_result$z, Ef = impute_result$Ef, Varf = impute_result$Varf, Yimp0 = impute, Y = impute_result$Y, pg = pg, geneM = impute_result$geneM,
            geneSd = impute_result$geneSd))
    } else {
        # gene_mean[lq_ind] = mu[lq_ind,] Y[lq_ind, ] = Y[lq_ind, ] - mu[lq_ind] mu[lq_ind] = 0
        ...
        return(list(loglik = impute_hq$loglik, pi = pi, mu = mu, sigma = sigma, beta = beta, lambda = lambda, z = impute_hq$z, Ef = impute_hq$Ef, Varf = impute_hq$Varf,
            Yimp0 = impute, Y = Y, pg = pg, geneM = gene_mean, geneSd = rep(1, G)))
    }
}
