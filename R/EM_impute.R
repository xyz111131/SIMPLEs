...
EM_impute <- function(Y, Y0, pg, M0, K0, cutoff, iter, beta, sigma, lambda, pi, z, mu = NULL, celltype = NULL, penl = 1, est_z = 2, max_lambda = T, est_lam = 2,
    ...
    for (it in 1:iter) {
        ...
        res <- foreach(g = 1:G, .combine = rbind) %dopar% {
            ...
            fit1m <- glmnet(W_aug, Y_aug, family = "gaussian", alpha = 1, intercept = F, standardize = F, nlambda = 1, lambda = mean(penalty)/(M0 * n + K0)/var(Y_aug),
            ...
            if (mt > 1) {
                ...
                for (k in 1:K0) {
                  if (all(beta[, k] == 0)) {
                    ...
                    lambda[ind_m, k] <- solnp(lam_init[, k], fun = fn, eqfun = equal, eqB = mt/M0, LB = rep(0, mt), UB = rep(mt/M0, mt), control = list(inner.iter = 500,
