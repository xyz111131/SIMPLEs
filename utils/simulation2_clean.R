...
for( r in 1:run)
{
  ...
  result1 = do_impute(Y2, result$Y, result$beta, result$lambda, result$sigma, result$mu, result$pi, result$geneM, result$geneSd, result$initclus, mcmc=50, burnin = 5, verbose=F, pg = result$pg, cutoff = 0.01) # only for AUC
  ...
  #### using bulk ####
  result <- scimpclu_bulk(Y2, K0, data.frame(simu_data$bulk), M0, celltype=rep(1, n), clus = NULL, K = 20, iter= 10, est_z = 1, impt_it = 1, max_lambda=T, est_lam = 2, penl =1, sigma0 = 100, p_min = pm, min_gene = 200,cutoff=0.01,verbose=T)
  resultYB = result$Y

  cluster_result[r,9:10] = mclust::adjustedRandIndex(apply(result$z,1, which.max), celltype_true) #getCluster(result$Yimp0, celltype_true, Ks = 20, M0 = M0)[[1]]
  mse[r,4] =mean((result$Yimp0[Y2==0] - Y[Y2==0])^2)

  result2 = do_impute(Y2, result$Y, result$beta, result$lambda, result$sigma, result$mu, result$pi, result$geneM, result$geneSd, rep(1, n), mcmc=50, burnin = 5, verbose=F, pg = result$pg, cutoff = 0.01)
...
save(cluster_result, mse, auc, mse_cor, file=paste("SIMPLE/Simulation/simulation_K6_K0",K0,"pm", pm ,"lowdrop.rdat", sep="_"))
