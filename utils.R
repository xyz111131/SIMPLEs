library(ggplot2)
simulation_bulk <- function(n = 300, S0 = 10, K = 3, MC=2, block_size=50, overlap = 15, indepG = 30, dropr=0.3, verbose=F)
{
  B = NULL
  W = NULL
  Lambda = NULL
  ...
  {
    ss = ((m-1) * S0 + 1) : (m * S0)
    Mu[act_ind[ss] ,m] = Mu[act_ind[ss],m] * sample(c(0.2, 0.5, 1.2, 1.5 , 2), S0, replace = T) #c(0.3, 0.5, 2 , 3, 4)
  ...
  if(K > 0)
  {
    ...
    if(K > 1)
    {
      for(i in 2:K)
      {
        Gamma[((block_size - overlap)*(i-1) +1) :(block_size + (block_size - overlap)*(i-1)), i] <- 1.0
      }
    }
  ...
  if(verbose)
  {
    if(K>0) plot(ggplot(melt(Gamma), aes(Var2,Var1)) +geom_tile(aes(fill = value)) +
                   scale_fill_gradient(low = "white", high = "red",guide="colourbar")+theme_bw()+xlab("")+ylab(""))
  ...
  return(list("Y2" = Y2, "Y" = Y, "B" = B, "W" = W, "Mu" = Mu, "Lambda" = Lambda, "Sigma" = Sigma,"Z"=Z, "bulk" = rowMeans(Mu[,Z]), "S_label" = label0)) #
}

getCluster<- function(Y2, celltype, Ks=NULL, M0=NULL)
{
  if(is.null(M0)) M0 = length(unique(celltype))
  if(is.null(Ks)) Ks = M0-1
  ...
  for(K in Ks)
  {
    ...
    result[i, ] = c(mclust::adjustedRandIndex(km$cluster, celltype),
                    mclust::adjustedRandIndex(km0$cluster, celltype))
    i = i +1
  }
...
# tsne plot
tsneplot <- function(x, celltype, pca = T, scale = T, center = T , K = 20, file=NULL)  # n*G, K
{

  result <- Rtsne::Rtsne(x, pca_scale = scale, pca_center = center,initial_dims = K, pca = pca)
...
mycor <- function(X, m, celltype, cors)
{
  ...
  return(c(mean(org_cors[cors==0]^2),
  mean((org_cors[(cors!=0) & (cors!=1)] - cors[(cors!=0) & (cors!=1)])^2)))
}
